          ...
          <section className="rounded-[2rem] border border-line bg-white p-5 shadow-soft sm:p-6">
            ...
            <form onSubmit={submit} className="grid gap-4">
              ...
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                <FormInput label="Nombre del viaje" value={form.name} onChange={(event) => update("name", event.target.value)} placeholder="Escapada a Lisboa" />
                <FormInput label="Destino" value={form.destination} onChange={(event) => update("destination", event.target.value)} placeholder="Lisboa, Portugal" />
                ...
                <FormInput label="Fechas" value={form.dates} onChange={(event) => update("dates", event.target.value)} placeholder="12 junio - 16 junio" />
                <FormInput label="Personas" type="number" min="1" value={form.people} onChange={(event) => update("people", event.target.value)} />
                <FormInput label="Presupuesto estimado" value={form.budget} onChange={(event) => update("budget", event.target.value)} placeholder="650" />
                <FormSelect label="Estado inicial" value={form.status} onChange={(event) => update("status", event.target.value)}>
                  <option>Idea</option>
