...
export default function ChecklistSection({ title, items, group, onToggle, onEdit, onDelete }) {
  const done = items.filter((item) => item.done).length;

  return (
    <SectionCard
      title={title}
      action={<span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-black text-slate-600">{done}/{items.length}</span>}
    >
